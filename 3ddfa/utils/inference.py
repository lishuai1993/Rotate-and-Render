...
def get_5lmk_from_68lmk(lmk68):
    ...
    nose = lmk68[33, :]
    left_mouse = lmk68[48, :]
    right_mouse = lmk68[54, :]
    lmk5 = [left_eye, right_eye, nose, left_mouse, right_mouse]
    return np.concatenate(lmk5)
...
def _predict_vertices(param, roi_bbox, dense, transform=True, align_pose=False):
    vertex = reconstruct_vertex(param, dense=dense, align_pose=align_pose)
...
def predict_68pts(param, roi_box, align_pose=False):
    return _predict_vertices(param, roi_box, dense=False, align_pose=align_pose)

def predict_dense(param, roi_box, align_pose=False):
    return _predict_vertices(param, roi_box, dense=True, align_pose=align_pose)

def get_aligned_param(param):
    Ps = param[:12].reshape(3, -1)
    s, R, t3d = P2sRt(Ps)
    Ps[:, :3] = np.identity(3) * s
    # t3d = t3d.reshape(3, -1)
    # p = p_[:, :3]
    # offset = p_[:, -1].reshape(3, 1)
    # print(R)
    # angles = matrix2angle(R) # yaw, pitch, roll
    # angles = [x, y, z]
    # if 'yaw' in change_axis:
    #     angles[0] = 0
    # if 'pitch' in change_axis:
    #     angles[1] = 0
    # if 'roll' in change_axis:
    #     angles[2] = 0
    # p = angle2matrix(angles)
    # print(p)
    # new_p_ = np.concatenate((p, t3d), axis=1).reshape(-1)
    param[:12] = Ps.reshape(-1)
    return param
